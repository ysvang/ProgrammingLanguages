// Author: Scott Vang
// Problem 15.2: Bulletin Board style with extension to return number of non-stop words with letter z.
...
function Z_Word_Counter(event_manager) {
    this._event_manager = event_manager;
    this.number_of_z_words = 0;
    var self = this;
    
    this.z_Word_Check = function (event) {
        var word = event[1];
        if(word.indexOf('z') != -1) {
            self.number_of_z_words += 1;
        }
    }
    
    this.print_number_z_words = function (event){
        console.log("The number of non-stop words with letter z is: " + self.number_of_z_words);
    };
    
    this._event_manager.subscribe('valid_word', this.z_Word_Check);
    this._event_manager.subscribe('print', this.print_number_z_words);
}
